...
describe('disconnectArtistWalletHandler', () => {
  ...
  describe('Privy path', () => {
    it('returns success and calls disconnectWallets with correct args', async () => {
      ...
      const res = await disconnectArtistWalletHandler({ method: AuthMethod.Privy, token: TOKEN });
      const json = await res.json();

      expect(json).toEqual({ success: true });
      expect(disconnectWallets).toHaveBeenCalledWith({ social_wallet: SOCIAL, external_wallet: EXTERNAL });
    });

    it('throws when socialWallet is missing', async () => {
      vi.mocked(getAddressesByPrivyToken).mockResolvedValue({ artistAddress: EXTERNAL, socialWallet: undefined });
      await expect(disconnectArtistWalletHandler({ method: AuthMethod.Privy, token: TOKEN })).rejects.toThrow('In*Process wallet not found');
    });

    it('throws when externalWallet is missing', async () => {
      vi.mocked(getAddressesByPrivyToken).mockResolvedValue({ artistAddress: undefined, socialWallet: SOCIAL });
      await expect(disconnectArtistWalletHandler({ method: AuthMethod.Privy, token: TOKEN })).rejects.toThrow('External wallet not found');
    });
  });

  describe('ApiKey path — caller is the social wallet (isPrivySocialWallet=true)', () => {
    beforeEach(() => {
      vi.mocked(authenticateWithApiKey).mockResolvedValue({ artistAddress: SOCIAL, authMethod: AuthMethod.ApiKey });
    ...
    it('returns success and calls disconnectWallets with correct args', async () => {
      vi.mocked(selectWallets)
        ...
        .mockResolvedValueOnce({ data: [{ address: EXTERNAL }], error: null } as any);

      const res = await disconnectArtistWalletHandler({ method: AuthMethod.ApiKey, token: TOKEN });
      const json = await res.json();
    ...
    it('throws when artistId not found', async () => {
      ...
      await expect(disconnectArtistWalletHandler({ method: AuthMethod.ApiKey, token: TOKEN })).rejects.toThrow('External wallet not found');
    });
  ...
  describe('ApiKey path — caller is the external wallet (isPrivySocialWallet=false)', () => {
    beforeEach(() => {
      vi.mocked(authenticateWithApiKey).mockResolvedValue({ artistAddress: EXTERNAL, authMethod: AuthMethod.ApiKey });
      vi.mocked(isPrivyWalletAddress).mockResolvedValue(false);
    });

    it('returns success and calls disconnectWallets with correct args', async () => {
      vi.mocked(selectArtists).mockResolvedValue({ data: [{ id: ARTIST_UUID }], error: null } as any);
      vi.mocked(selectWallets).mockResolvedValue({ data: [{ address: SOCIAL }], error: null } as any);
    ...
    it('throws when artist not found', async () => {
      vi.mocked(selectArtists).mockResolvedValue({ data: [], error: null } as any);
      await expect(disconnectArtistWalletHandler({ method: AuthMethod.ApiKey, token: TOKEN })).rejects.toThrow('Artist not found');
    });

    it('throws when social wallet not found', async () => {
      vi.mocked(selectArtists).mockResolvedValue({ data: [{ id: ARTIST_UUID }], error: null } as any);
      vi.mocked(selectWallets).mockResolvedValue({ data: [], error: null } as any);
      await expect(disconnectArtistWalletHandler({ method: AuthMethod.ApiKey, token: TOKEN })).rejects.toThrow('In*Process wallet not found');
    });
  });
});
