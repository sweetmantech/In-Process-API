...
export async function createMoment(
  ...
  if (resolvedSplits && resolvedSplits.length >= 2) {
    ...
    if (result.splitAddress) {
      ...
      payoutRecipient = getAddress(
        splitAddress
      ) as typeof input.token.payoutRecipient;
    }
  }
  ...
  let additionalSetupActions:
    | ((args: { tokenId: bigint }) => Hex[])
    | undefined;

  if (
    (resolvedSplits && resolvedSplits.length > 0) ||
  ...
  const isNewContractByAddress =
    getAddress(parameters.address) === getAddress(factoryAddress);
