...
export const executeAirdrop = async ({
  ...
  const recipients = Array.from({ length: airdropToItems.length }).map(
    (_, i) => ({
  ...
  const smartWalletPermission = await getPermission(
  ...
  if (smartWalletPermission !== BigInt(PERMISSION_BIT_ADMIN)) {
    ...
    const artistWalletPermission = await getPermission(
      moment.collectionAddress,
      artistWallet
    );
