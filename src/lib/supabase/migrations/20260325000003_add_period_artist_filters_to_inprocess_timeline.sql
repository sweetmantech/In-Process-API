-- Add p_period, p_channel, and p_artist filters to get_in_process_timeline
  ...
  WITH filtered_moments AS (
    ...
    WHERE
      ...
      AND (
        p_period IS NULL
        OR p_period = 'all'
        OR m.created_at >= NOW() - CASE p_period
      ...
      AND (
        p_artist IS NULL
        OR da.username ILIKE p_artist
        OR da.address = LOWER(p_artist)
      )
      AND (
        p_hidden = true
        OR NOT EXISTS (
