...
describe('registerOnStart', () => {
  it('registers a handler for the /start pattern', () => {
    const bot = makeBot();
    registerOnStart(bot as any);
    expect(bot.onNewMessage).toHaveBeenCalledOnce();
    expect(bot.getPattern()).toEqual(/^\/start$/);
  });
  ...
  it('posts the welcome message', async () => {
    ...
    expect(message).toContain('Welcome to In-Process Bot!');
