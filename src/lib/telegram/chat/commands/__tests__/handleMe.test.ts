...
describe('handleMe', () => {
  describe('artistAddress has a known artist record', () => {
    it('posts the linked email found via privy wallet', async () => {
      ...
      vi.mocked(selectWallets)
        .mockResolvedValueOnce({ data: [{ address: SOCIAL_WALLET }], error: null } as any)
      ...
      expect(selectWallets).toHaveBeenCalledWith({ artistIds: [ARTIST_UUID], type: 'privy' });
      expect(getEmailByWalletAddress).toHaveBeenCalledWith(SOCIAL_WALLET);
      expect(thread.post).toHaveBeenCalledWith('Your linked email: user@example.com');
    });
    ...
    it('tries each privy wallet and returns the first email found', async () => {
      ...
      expect(thread.post).toHaveBeenCalledWith('Your linked email: second@example.com');
    });
    ...
    it('falls through to direct address check when no privy wallet has an email', async () => {
      ...
      vi.mocked(selectWallets)
        .mockResolvedValueOnce({ data: [{ address: SOCIAL_WALLET }], error: null } as any)
        .mockResolvedValueOnce({ data: [{ type: 'privy' }], error: null } as any);
      ...
      expect(thread.post).toHaveBeenCalledWith('No email address linked to your account.');
    });
  });
  ...
  describe('artistAddress is itself a privy wallet', () => {
    it('posts the email found directly from the privy wallet address', async () => {
      ...
      vi.mocked(selectWallets).mockResolvedValue({ data: [{ type: 'privy' }], error: null } as any);
      vi.mocked(getEmailByWalletAddress).mockResolvedValue('social@example.com');
      ...
      expect(thread.post).toHaveBeenCalledWith('Your linked email: social@example.com');
    });
  ...
  describe('error handling', () => {
    it('rethrows on selectArtists error', async () => {
      ...
      const thread = makeThread();
      await expect(handleMe(thread as never, ARTIST_ADDRESS)).rejects.toThrow('db down');
    });
    ...
    it('rethrows on selectWallets error', async () => {
      ...
      const thread = makeThread();
      await expect(handleMe(thread as never, ARTIST_ADDRESS)).rejects.toThrow('lookup failed');
    });
  });
});
