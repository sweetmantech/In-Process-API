...
describe('validateMediaStream', () => {
  ...
  describe('origin error propagation', () => {
    it('should propagate 404 from origin', async () => {
      ...
      const request = createMockRequest(
        'https://api.example.com/media/stream?url=https://example.com/notfound.mp4'
      );
      const result = await validateMediaStream(request);
    ...
    it('should propagate 401 from origin', async () => {
      ...
      const request = createMockRequest(
        'https://api.example.com/media/stream?url=https://example.com/private.mp4'
      );
      const result = await validateMediaStream(request);
    ...
    it('should propagate 500 from origin', async () => {
      ...
      const request = createMockRequest(
        'https://api.example.com/media/stream?url=https://example.com/error.mp4'
      );
      const result = await validateMediaStream(request);
  ...
  describe('successful validation', () => {
    it('should return validated data for https URL', async () => {
      ...
      expect(result).toEqual({
        fetchableUrl: 'https://example.com/video.mp4',
        totalSize: 12345,
        supportsRange: true,
        contentType: 'video/mp4',
        rangeHeader: null,
    ...
    it('should return validated data for ar:// URL', async () => {
      vi.mocked(getContentInfo).mockResolvedValue({
        ...
        supportsRange: true,
        contentType: 'video/mp4',
        finalUrl: 'https://arweave.net/abc123',
      });
    ...
    it('should use final URL after redirect', async () => {
      vi.mocked(getContentInfo).mockResolvedValue({
        ok: true,
        totalSize: 54321,
        supportsRange: true,
        contentType: 'video/mp4',
        finalUrl: 'https://cdn.arweave.net/abc123',
      });
      ...
      expect(result).not.toBeInstanceOf(NextResponse);
      expect(result).toEqual({
        fetchableUrl: 'https://cdn.arweave.net/abc123',
        totalSize: 54321,
        supportsRange: true,
        contentType: 'video/mp4',
        rangeHeader: null,
